...
def extract_path_features(paths, feature_dict, svg_width, svg_height):
    ...
    for path in paths:
        ...
        if len(path_numbers) > 3:
            ...
            for (start, end, max_distance, start_list, count_dict) in [
                (start_x, end_x, max_x_distance, x_list, x_count), (start_y, end_y, max_y_distance, y_list, y_count)]:
...
def get_attributes(path):
    ...
    for attr, include_parent, keywords in [('stroke', True, ['line']), ('fill', True, ['area', 'shape']),
                                           ('stroke-width', False, [])]:
...
def is_hexagon(d):
    ...
    if d[-1] == 'z':
        ...
        return abs(distance2(x1, y1, x2, y2) - distance2(x2, y2, x3, y3)) < 0.1 * distance2(x1, y1, x2, y2) and abs(
            distance2(x2, y2, x3, y3) - distance2(x3, y3, x4, y4)) < 0.1 * distance2(x2, y2, x3, y3) and abs(
            distance2(x3, y3, x4, y4) - distance2(x4, y4, x5, y5)) < 0.1 * distance2(x3, y3, x4, y4)
    return False
